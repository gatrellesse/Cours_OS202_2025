...
nbp = glob_comm.Get_size()
rank = glob_comm.Get_rank()
name = MPI.Get_processor_name()

comp_comm = glob_comm.Split(rank!=0, rank)
...
class Grille:
    ...
    def update_ghost_cells(self):
        requests.append(comp_comm.isend(grid.cells[1,:], dest = (comp_rank - 1 + comp_nbp)%comp_nbp))
        requests.append(comp_comm.isend(grid.cells[-2,:], dest = (comp_rank + 1)%comp_nbp))
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if(rank != 0):
            ...
            glob_comm.send(diff, dest = 0)
            #ghostCells
            comp_comm.Send(grid.cells[1,:], dest = (comp_rank - 1 + comp_nbp)%comp_nbp)
            comp_comm.Send(grid.cells[-2,:], dest = (comp_rank + 1)%comp_nbp)

            req1 = comp_comm.Irecv(grid.cells[0,:], source = (comp_rank - 1 + comp_nbp)%comp_nbp)
            req2 = comp_comm.Irecv(grid.cells[-1,:], source = (comp_rank + 1)%comp_nbp)
            ...
            print(f"Temps calcul prochaine generation : {t2-t1:2.2e}")
        ...
        if(rank == 0):
            ...
            appli.setDiff(diff, source)

            appli.draw()
            t3 = time.time()
            print(f"Temps affichage : {t3-t1:2.2e} secondes\r", end='')
        # time.sleep(10) # A régler ou commenter pour vitesse maxi

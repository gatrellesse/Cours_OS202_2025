...
#to run mpirun -np 2 python3 TASK_1_game_of_life.py 
...
if __name__ == '__main__':
    ...
    while loop:
        ...
        if(rank != 0):
            ...
            #comm.send(gird, dest=1, tag = 1) 
            comm.send(diff, dest=MASTER_RANK, tag = 1) 
            t2 = time.time()
            print(f"Temps  calcul prochaine generation : {t2-t1:2.2e} secondes\r", end='')

        if(rank == 0):
            diff_cells = comm.recv(source=1, tag = 1)
            t2 = time.time()
            #comm.Recv(appli.grid.cells,source=1, tag=1)
            ###comm.Recv(appli.grid.data, tag = 1)
            appli.grid.cells = diff_cells
            appli.draw()
            t3 = time.time()
